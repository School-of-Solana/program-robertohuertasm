...
import {
  ...
  useSimpleVotingAppProgram,
  useSimpleVotingAppProgramAccount,
  useDeletePoll,
} from './voting-data-access';
import { BN } from 'bn.js';

export function SimpleVotingAppCreate() {
  const { initializePollAndCandidates, isPending } = useInitializePollAndCandidates();

  const { polls } = useSimpleVotingAppProgram();
...
export function SimpleVotingAppList() {
  const { polls, programAccount } = useSimpleVotingAppProgram();
...
function PollCard({ pollAddress }: { pollAddress: PublicKey }) {
  const { pollQuery, voteMutation } = useSimpleVotingAppProgramAccount({
    pollAddress,
  });
      ...
      <div className="card-body items-center text-center">
        <div className="space-y-6">
          ...
          <div className="text-center space-y-4 pt-10">
            ...
            <button
              className="btn btn-xs lg:btn-md btn-error mt-4"
              onClick={() => {
                const pollId = pollQuery.data?.poll.pollId;
                const candidateAddresses = pollQuery.data?.candidates.map((c) => c.publicKey) ?? [];
                if (!pollId) return;
                deletePollMutation.mutateAsync({
                  pollId,
                  pollAddress,
                  candidateAddresses,
                });
              }}
              disabled={deletePollMutation.isPending || !pollQuery.data?.poll.pollId}
            >
              Delete Poll
            </button>
          </div>
