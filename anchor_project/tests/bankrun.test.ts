...
const IDL = require('../target/idl/simple_voting_app.json');
const PROGRAM_ID = new PublicKey(IDL.address);

describe('simple_voting_app.bankrun.test', () => {
  async function setup() {
    const context = await startAnchor(
      '',
      [{ name: 'simple_voting_app', programId: PROGRAM_ID }],
      [],
    );
    const provider = new BankrunProvider(context);

    const payer = provider.wallet as anchor.Wallet;
    const program = new anchor.Program<SimpleVotingApp>(IDL, provider);
  ...
  function localValidatorSetup() {
    ...
    program = anchor.workspace.SimpleVotingApp as Program<SimpleVotingApp>;
    const provider = anchor.AnchorProvider.env();
    anchor.setProvider(provider);
  }

  let program: Program<SimpleVotingApp>;
